...
function mergeCartEntries(items) {
  ...
  for (const item of Array.isArray(items) ? items : []) {
    ...
    const nextItem = { ...item, qty: Number(item.qty || 1) };
    ...
    const nextQty = Number(nextItem.qty || 1);
    existing.qty = Math.max(existingQty, nextQty);
    existing.price = Number(nextItem.price || existing.price || 0);
    if (nextItem.name) existing.name = nextItem.name;
    if (nextItem.product_type) existing.product_type = nextItem.product_type;
    if (nextItem.product_subtype) existing.product_subtype = nextItem.product_subtype;
    if (nextItem.cover_image_url) existing.cover_image_url = nextItem.cover_image_url;
  }
